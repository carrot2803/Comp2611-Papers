...
// you could just find maxBst use the parent and move around for two cases
BTNode *deleteMaxBST(BTNode *root) {
    if (root == NULL) return NULL;
    BTNode *minBST = maximumBST(root);
...
int main() {
    ...
    root->right->right = new BTNode{200, nullptr, nullptr};

    cout << "Original BST:" << endl;
    inOrder(root);

    root = deleteMaxBST(root);

    cout << "\nBST after deleting the maximum value node:" << endl;
    inOrder(root);
